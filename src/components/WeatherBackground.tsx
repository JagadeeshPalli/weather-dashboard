...
/* Condition tint — shifts the whole background per condition */
...
/* ── Pre-computed star positions (golden-angle distribution, top 65% of sky) */
    ...
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      {STARS.map((s, i) => (
        <motion.div
          ...
          animate={{ opacity: [s.bright, s.bright * 0.18, s.bright] }}
...
/* ─── Aurora borealis (clear dark, very subtle) ──────────────────────────── */
function AuroraEffect() {
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none">
      <motion.div
        style={{
          position: 'absolute', top: '-18%', left: '-12%', right: '-12%', height: '52%',
      ...
      <motion.div
        style={{
          position: 'absolute', top: '2%', left: '15%', right: '-22%', height: '44%',
      ...
      <motion.div
        style={{
          position: 'absolute', top: '-5%', left: '-20%', right: '20%', height: '38%',
...
/* ─── Moon orb with halo rings (dark/clear) ──────────────────────────────── */
function MoonOrb() {
  return (
    <div className="absolute top-[-55px] right-[-35px] w-[280px] h-[280px] pointer-events-none">
      ...
      <div
        ...
        style={{ background: 'rgba(203,213,225,0.12)' }}
      />
      {/* Halo rings */}
      {[115, 85, 62].map((r, i) => (
        <motion.div
  ...
  const drops = useMemo(
    () =>
      Array.from({ length: count }, (_, i) => ({
        ...
        left:     `${-2 + (i / count) * 106}%`,
        delay:    (i * 0.048) % 2.2,
        duration: heavy ? 0.42 + (i % 5) * 0.04 : 0.66 + (i % 5) * 0.07,
    ...
    <div style={{ transform: `skewX(${windAngleDeg}deg)`, position: 'absolute', inset: 0, overflow: 'hidden' }}>
      {drops.map((d) => (
        <motion.div
          key={d.id}
          className="absolute rounded-full"
          style={{ left: d.left, top: '-6%', height: d.height, width: d.width, background: grad, opacity: d.opacity }}
          animate={{ y: '114vh' }}
